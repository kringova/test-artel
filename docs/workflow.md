...
> **Адаптер-специфика.** Конкретный способ *фиксировать* стоимость и *механически блокировать* прыжок — дело адаптера (напр., Claude-адаптер llmush делает это через `scripts/task-checkpoint.py` + pre-commit-хук). Канон Артели — только маркер `doing`: он существует, он ставится до старта, он виден на борде.
